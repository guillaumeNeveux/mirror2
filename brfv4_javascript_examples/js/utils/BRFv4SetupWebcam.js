...
(function(){
	...
	webcam.setupStream = function(video, width, height, fps, callback) {
		...
		brfv4.trace("webcam.setupStream: isPlaying: " + webcam.isPlaying);
	...
	webcam.startStream = function() {
		...
		brfv4.trace("webcam.startStream: try: " +
	...
	webcam.stopStream = function() {
		...
		if(webcam.isPlaying) {
			brfv4.trace("webcam.stopStream: isPlaying: " + webcam.isPlaying);
		}
	...
	webcam.onStreamDimensionsAvailable = function() {
		...
		if(webcam.video.videoWidth == 0) {
			...
			brfv4.trace("webcam.onStreamDimensionsAvailable: waiting");
			...
			brfv4.trace("webcam.onStreamDimensionsAvailable: " + webcam.video.videoWidth + "x" + webcam.video.videoHeight);
	...
	webcam.onStreamError = function(event) {
		brfv4.trace("webcam.onStreamError: " + event);
	...
	webcam.setup = function(webcamVideo, imageDataCanvas, resolution, onImageDataReady) {
		...
		if(!webcamVideo || !imageDataCanvas) {
			brfv4.trace("setupWebcam: Please add a <video> tag with id='_webcam' and " +
				"a <canvas> tag with id='_imageData' to the DOM.", true);
			return;
		}

		if(!resolution) {
			brfv4.trace("setupWebcam: Please setup a resolution Rectangle.", true);
			return;
		}
